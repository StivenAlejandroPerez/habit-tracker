...
func TestEventRepository_UpdateEvents(t *testing.T) {
	type fields struct {
		db Drivers
	}
	type args struct {
...
SET
	habit_id = 2,
	subject = 'Go to gym',
	start_at = '2023-07-27T12:00:00Z',
	end_at = '2023-07-27T14:00:00Z',
...
SET
	habit_id = 3,
	subject = 'Painting class',
	start_at = '2023-07-27T14:00:00Z',
	end_at = '2023-07-27T16:00:00Z',
	updated_at = '2023-07-20T15:32:00Z'
WHERE
	id = 2;`
	tests := []test{
		func() test {
			...
			mock.ExpectExec(regexp.QuoteMeta(query)).WillReturnResult(sqlmock.NewResult(2, 2))
	...
	for _, tt := range tests {
		t.Run(
			...
			func(t *testing.T) {
				...
				tt.wantErr(t, er.UpdateEvents(tt.args.ctx, tt.args.events, tt.args.now))
			},
		)
	}
}
...
func Test_buildUpdateEventsQuery(t *testing.T) {
	type args struct {
		events habit_tracker.Events
		now    time.Time
	}
...
WHERE
	...
	for _, tt := range tests {
		t.Run(
			tt.name,
			func(t *testing.T) {
				assert.Equal(t, tt.want, buildUpdateEventsQuery(tt.args.events, tt.args.now))
			},
		)
	}
}
